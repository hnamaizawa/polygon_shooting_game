(() => {
  ...
  let ctx = null, master = null, bgmBus = null, sfxBus = null, bgmTimer = null, step = 0, muted = false;

  const melody = [
    69,72,76,81,79,76,72,74, 76,79,83,79,76,74,72,69,
    69,72,76,81,84,81,79,76, 74,76,79,83,81,79,76,72
  ];
  const harmony = [
    64,64,67,67,65,65,67,67, 64,64,67,67,65,65,64,64,
    64,64,67,67,69,69,67,67, 65,65,67,67,64,64,62,64
  ];
  const bass = [
    45,45,45,45,41,41,43,43, 45,45,45,45,41,41,40,40,
    45,45,45,45,48,48,43,43, 41,41,43,43,40,40,45,45
  ];
  ...
  function ensure() {
    ...
    if (!ctx) {
      ...
      master.gain.value = 0.68; bgmBus.gain.value = 0.34; sfxBus.gain.value = 0.50;
  ...
  function tone(freq, duration, type='square', gain=.08, when=0, bus=sfxBus, endFreq=null) {
    ...
    amp.gain.setValueAtTime(.0001, t); amp.gain.exponentialRampToValueAtTime(Math.max(.0002, gain), t + .008); amp.gain.exponentialRampToValueAtTime(.0001, t + duration);
    osc.connect(amp); amp.connect(bus || sfxBus); osc.start(t); osc.stop(t + duration + .02);
  }
  function noise(duration=.18, gain=.12, cutoff=900) {
    ...
    filter.type='lowpass';filter.frequency.value=cutoff;amp.gain.setValueAtTime(gain,ctx.currentTime);amp.gain.exponentialRampToValueAtTime(.0001,ctx.currentTime+duration);
    src.buffer=buffer;src.connect(filter);filter.connect(amp);amp.connect(sfxBus);src.start();src.stop(ctx.currentTime+duration+.02);
  }
  function scheduleBgmStep() {
    if (!ctx) return;
    const i=step%melody.length,m=melody[i],h=harmony[i],b=bass[i];
