(() => {
  ...
  function makeGlossMetalTexture(base, accent, dark, cool='#edf7ff') {
    ...
    baseGrad.addColorStop(0,'#0b1016');baseGrad.addColorStop(.12,dark);baseGrad.addColorStop(.31,base);
    baseGrad.addColorStop(.44,'#fbfdff');baseGrad.addColorStop(.50,cool);baseGrad.addColorStop(.58,'#c8d0d8');
    baseGrad.addColorStop(.67,base);baseGrad.addColorStop(.84,'#53606d');baseGrad.addColorStop(1,dark);
    g.fillStyle=baseGrad;g.fillRect(0,0,96,96);
    const shine=g.createLinearGradient(0,0,96,0);
    shine.addColorStop(0,'rgba(255,255,255,0)');shine.addColorStop(.36,'rgba(255,255,255,.05)');
    shine.addColorStop(.47,'rgba(255,255,255,.72)');shine.addColorStop(.52,'rgba(255,255,255,.98)');
    shine.addColorStop(.59,'rgba(255,255,255,.14)');shine.addColorStop(1,'rgba(255,255,255,0)');
    g.fillStyle=shine;g.fillRect(0,0,96,96);
    ...
    for(let i=0;i<38;i++){const x=(i*29+7)%92,y=(i*17+13)%92;g.fillRect(x,y,1+(i%3===0?2:1),1);}
    g.fillStyle='rgba(0,0,0,.22)';
    for(let i=0;i<20;i++){const x=(i*19+11)%90,y=(i*37+5)%90;g.fillRect(x,y,5,1);}
    return tex;
  }
  ...
  const turretModel={
    vertices:[[-1.25,.15,-1.0],[1.25,.15,-1.0],[1.15,.15,1.0],[-1.15,.15,1.0],[-.75,-.55,-.55],[.75,-.55,-.55],[.65,-.55,.65],[-.65,-.55,.65],[0,-.95,.1],[0,-1.15,2.25]],
    faces:[{v:[0,1,2],s:.12},{v:[0,2,3],s:.16},{v:[4,5,6],s:.06},{v:[4,6,7],s:.09},{v:[0,4,7],s:.22},{v:[0,7,3],s:.18},{v:[1,2,6],s:.20},{v:[1,6,5],s:.16},{v:[4,5,8],s:.04},{v:[5,8,9],s:.08},{v:[8,7,9],s:.11}]
  };
  const bossModel={
    vertices:[[0,-.8,5.2],[0,-1.6,1.2],[0,.6,.8],[-2.2,-.2,2.2],[-5.0,.4,.5],[-6.5,.7,-2.0],[-2.4,.5,-2.8],[2.2,-.2,2.2],[5.0,.4,.5],[6.5,.7,-2.0],[2.4,.5,-2.8],[0,.1,-4.5],[-3.2,-.4,-1.0],[3.2,-.4,-1.0]],
    faces:[{v:[0,1,3],s:.02},{v:[0,7,1],s:.04},{v:[0,3,2],s:.08},{v:[0,2,7],s:.10},{v:[3,4,5],s:.14},{v:[3,5,6],s:.19},{v:[7,10,9],s:.17},{v:[7,9,8],s:.13},{v:[2,6,11],s:.22},{v:[2,11,10],s:.20},{v:[3,12,6],s:.08},{v:[7,10,13],s:.08},{v:[1,2,3],s:.06},{v:[1,7,2],s:.06}]
  };
  ...
  function spawnBoss(){
    ...
    state.bossSpawned=true;state.boss=C.makeBoss(state.stage);state.bannerTimer=2.2;A.playBoss();
  }
  ...
  function finishGame(){
    ...
    startPanel.innerHTML=`<h1>ALL STAGES CLEARED</h1><p>SCORE ${String(state.score).padStart(7,'0')}</p><p>R / Enter で再出撃</p><button id="restartButton">RESTART</button>`;
  ...
  function endGame(){
    ...
    startPanel.innerHTML=`<h1>GAME OVER</h1><p>STAGE ${state.stage}</p><p>SCORE ${String(state.score).padStart(7,'0')}</p><p>R / Enter で再出撃</p><button id="restartButton">RETRY</button>`;
    startPanel.classList.remove('hidden');document.getElementById('restartButton').addEventListener('click',start);
  }
  ...
  function update(dt){
    ...
    for(const e of state.enemies){
      for(const l of state.lasers){
        if(e.alive&&l.z<900&&C.spheresHit(e,l,1.15,2.8)){e.alive=false;l.z=999;state.score+=C.scoreForEnemy(e);burst(e.x,e.y,e.z);A.playExplosion();}
      }
      if(e.alive&&C.spheresHit(e,state.player,1.7,3.8)){e.alive=false;state.lives--;state.shake=1;burst(state.player.x,state.player.y,state.player.z+1,22);A.playHit();if(state.lives<=0)endGame();}
    ...
    for(const g of state.groundEnemies){
      for(const l of state.lasers){
        if(g.alive&&l.z<900&&C.planarHit(g,l,1.45,3.0)){
          l.z=999;g.hp--;burst(g.x,g.y,g.z,7);
          if(g.hp<=0){g.alive=false;state.score+=C.scoreForEnemy(g);burst(g.x,g.y,g.z,18);A.playExplosion();}
        }
      }
    ...
    if(state.boss?.alive){
      ...
      for(const l of state.lasers){
        if(l.z<900&&C.planarHit(state.boss,l,2.8,4.4)){
          l.z=999;state.boss.hp--;burst(l.x,C.CONFIG.flightPlaneY,state.boss.z,4);
          if(state.boss.hp<=0){
            state.boss.alive=false;state.score+=C.scoreForEnemy(state.boss);burst(state.boss.x,state.boss.y,state.boss.z,55);A.playExplosion();state.stageClearTimer=2.6;state.bannerTimer=2.6;
          }
        }
      }
      if(state.boss.alive&&state.hitCooldown<=0&&C.spheresHit(state.boss,state.player,3.5,5.2)){state.lives--;state.hitCooldown=1.5;state.shake=1.2;A.playHit();state.player.z=C.CONFIG.playerMinZ;if(state.lives<=0)endGame();}
    }
  ...
  function drawTexturedTriangle(texture,p0,p1,p2,uv0=UV[0],uv1=UV[1],uv2=UV[2],shade=0,gloss=.35){
    ...
    ctx.save();ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.closePath();ctx.clip();ctx.transform(a,b,c,d,e,f);ctx.drawImage(texture,0,0);ctx.restore();
    ...
    if(gloss>0){const sweep=((state?.elapsed||0)*120)%PLAY_W,grad=ctx.createLinearGradient(sweep-90,0,sweep+90,0);grad.addColorStop(0,'rgba(255,255,255,0)');grad.addColorStop(.46,'rgba(255,255,255,0)');grad.addColorStop(.50,`rgba(255,255,255,${gloss})`);grad.addColorStop(.56,'rgba(255,255,255,.05)');grad.addColorStop(1,'rgba(255,255,255,0)');ctx.fillStyle=grad;ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.closePath();ctx.fill();}
    ctx.strokeStyle='rgba(245,250,255,.24)';ctx.lineWidth=.8;ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.closePath();ctx.stroke();
  }
  function drawMesh(model,obj,scale,texture,gloss=.42){
    const pts=model.vertices.map(v=>projectWorld(obj.x+v[0]*scale,obj.y+v[1]*scale,obj.z+v[2]*scale));
    const faces=model.faces.map(face=>({face,depth:face.v.reduce((sum,i)=>sum+pts[i].depth,0)/3})).filter(item=>item.face.v.every(i=>pts[i].depth>.25)).sort((a,b)=>b.depth-a.depth);
    for(const item of faces){const[a,b,c]=item.face.v;drawTexturedTriangle(texture,pts[a],pts[b],pts[c],UV[0],UV[1],UV[2],item.face.s||0,gloss);}
  }
  ...
  function drawEarthBackdrop(){
    const x=125,y=86,r=155;
    const glow=ctx.createRadialGradient(x-28,y-38,18,x,y,r+24);glow.addColorStop(0,'rgba(220,248,255,.95)');glow.addColorStop(.18,'rgba(75,155,255,.92)');glow.addColorStop(.72,'rgba(15,64,142,.96)');glow.addColorStop(1,'rgba(0,12,34,0)');ctx.fillStyle=glow;ctx.beginPath();ctx.arc(x,y,r+24,0,Math.PI*2);ctx.fill();
    const earth=ctx.createRadialGradient(x-40,y-45,16,x,y,r);earth.addColorStop(0,'#c7f3ff');earth.addColorStop(.16,'#3d9cf0');earth.addColorStop(.72,'#124a9b');earth.addColorStop(1,'#051938');ctx.fillStyle=earth;ctx.beginPath();ctx.arc(x,y,r,0,Math.PI*2);ctx.fill();
    ctx.fillStyle='rgba(72,126,78,.78)';ctx.beginPath();ctx.ellipse(x-35,y-18,46,22,-.45,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.ellipse(x+38,y+24,38,17,.35,0,Math.PI*2);ctx.fill();
    ctx.strokeStyle='rgba(255,255,255,.32)';ctx.lineWidth=6;ctx.beginPath();ctx.arc(x,y,r-5,-2.4,-.65);ctx.stroke();
  }
  function drawSpaceBackdrop(){
    const g=ctx.createRadialGradient(PLAY_W*.72,H*.18,10,PLAY_W*.72,H*.18,310);g.addColorStop(0,'rgba(80,40,120,.24)');g.addColorStop(.45,'rgba(30,40,95,.12)');g.addColorStop(1,'rgba(0,0,0,0)');ctx.fillStyle=g;ctx.fillRect(0,0,PLAY_W,H);
  }
  function drawCombatPlatform(texture=textures.carrier){
    const scroll=(state?.elapsed||0)*5.0%22,z0=14-scroll,y=C.CONFIG.groundPlaneY+.28;
    drawWorldTexturedQuad([[-10.5,y,z0],[10.5,y,z0],[8.6,y,112],[-8.6,y,112]],texture,.15,.24);
  ...
  function drawCapitalShip(){
    ...
    drawWorldTexturedQuad([[-12,7.45,z0],[12,7.45,z0],[10.2,7.35,112],[-10.2,7.35,112]],textures.carrier,.12,.28);
    drawWorldTexturedQuad([[-12,7.45,z0],[-10.2,7.35,112],[-8.6,5.95,108],[-10.5,6.15,z0+2]],textures.carrier,.26,.18);
    drawWorldTexturedQuad([[12,7.45,z0],[10.5,6.15,z0+2],[8.6,5.95,108],[10.2,7.35,112]],textures.carrier,.22,.18);
    drawWorldPolygon([[-1.05,7.28,z0+1],[1.05,7.28,z0+1],[.55,7.20,112],[-.55,7.20,112]],'#071018','#7a95aa',1);
    for(let z=z0+5;z<114;z+=9){drawWorldPolygon([[-.72,7.17,z],[-.32,7.17,z],[-.24,7.15,z+2.8],[-.58,7.15,z+2.8]],'#b6e8ff');drawWorldPolygon([[.32,7.17,z],[.72,7.17,z],[.58,7.15,z+2.8],[.24,7.15,z+2.8]],'#b6e8ff');}
    for(let z=z0;z<114;z+=12){drawWorldTexturedQuad([[-9.6,7.25,z+1],[-5.3,7.25,z+1],[-5.0,7.20,z+8],[-9.0,7.20,z+8]],textures.carrier,.10,.34);drawWorldTexturedQuad([[5.3,7.25,z+1],[9.6,7.25,z+1],[9.0,7.20,z+8],[5.0,7.20,z+8]],textures.carrier,.13,.34);}
  }
  function drawInterior(){
    const scroll=(state?.elapsed||0)*6.2%16,z0=12-scroll;
    drawWorldTexturedQuad([[-10.5,7.2,z0],[10.5,7.2,z0],[8.5,7.0,116],[-8.5,7.0,116]],textures.interior,.13,.30);
    drawWorldTexturedQuad([[-10.5,7.2,z0],[-8.5,7.0,116],[-7.5,-2.5,116],[-11.5,-2.5,z0]],textures.interior,.26,.18);
    drawWorldTexturedQuad([[10.5,7.2,z0],[11.5,-2.5,z0],[7.5,-2.5,116],[8.5,7.0,116]],textures.interior,.23,.18);
    for(let z=z0+4;z<116;z+=10){drawLine3D([-10.2,6.9,z],[10.2,6.9,z],'rgba(255,90,90,.24)',2);drawLine3D([-10.0,-1.6,z],[10.0,-1.6,z],'rgba(180,205,225,.18)',1);}
    for(const x of[-7.5,-3.7,3.7,7.5])drawLine3D([x,7.0,z0],[x*.78,6.8,116],'rgba(220,235,245,.22)',1);
  }
  function drawStageBackdrop(){
    const s=currentStage();
    if(s.backdrop==='earth'){drawEarthBackdrop();drawCombatPlatform(textures.carrier);}
    else if(s.backdrop==='space'){drawSpaceBackdrop();drawCombatPlatform(textures.carrier);}
    else if(s.backdrop==='carrier')drawCapitalShip();
    else drawInterior();
  }
  ...
  function drawPlayfield(){
    ...
    for(const s of state.stars){const p=projectWorld(s.x,s.y,s.z);if(p.depth<=.2)continue;const r=Math.max(.5,Math.min(2.2,14/p.depth));ctx.globalAlpha=s.b;ctx.fillStyle='#fff';ctx.fillRect(p.x,p.y,r,r);}ctx.globalAlpha=1;
    drawStageBackdrop();
    drawRetroGrid();
