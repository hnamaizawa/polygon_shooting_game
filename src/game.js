(() => {
  ...
  function makeMetalTexture(base, accent, dark) {
    ...
    const g = tex.getContext('2d'); g.imageSmoothingEnabled = false;
    const grad = g.createLinearGradient(0,0,64,64);
    grad.addColorStop(0,dark); grad.addColorStop(.18,base); grad.addColorStop(.42,'#d7dde4');
    grad.addColorStop(.55,base); grad.addColorStop(.82,'#657180'); grad.addColorStop(1,dark);
    g.fillStyle=grad; g.fillRect(0,0,64,64);
    g.strokeStyle='rgba(20,25,32,.78)'; g.lineWidth=2;
    ...
    g.fillStyle=accent; g.fillRect(5,7,23,4);g.fillRect(35,21,22,4);g.fillRect(9,39,46,3);g.fillRect(28,47,8,11);
    g.fillStyle='rgba(255,255,255,.55)';g.fillRect(2,2,60,2);g.fillRect(3,18,26,1);g.fillRect(34,34,26,1);
    g.fillStyle='rgba(255,255,255,.22)';
    for(let i=0;i<30;i++){const x=(i*17+5)%62,y=(i*29+9)%62;g.fillRect(x,y,1+(i%2),1);}
    g.fillStyle='rgba(0,0,0,.28)';
    for(let i=0;i<16;i++){const x=(i*23+11)%61,y=(i*13+7)%61;g.fillRect(x,y,3,1);}
    return tex;
  }
  ...
  function reset(){
    state={running:true,gameOver:false,score:0,area:1,lives:3,elapsed:0,spawnTimer:.4,spawnBase:1.05,enemyId:1,fireTimer:0,shake:0,
      player:{x:0,y:3.25,z:C.CONFIG.playerZ},enemies:[],lasers:[],particles:[],stars:Array.from({length:105},()=>makeStar(true))};
  }
  function makeStar(randomZ=false){return{x:C.rand(-23,23),y:C.rand(-16,16),z:randomZ?C.rand(7,95):95,speed:C.rand(13,28),b:C.rand(.28,.85)};}
  ...
  function update(dt){
    ...
    if((keys.has('Space')||keys.has('KeyJ'))&&state.fireTimer<=0){
      state.lasers.push({x:state.player.x-.50,y:state.player.y-.15,z:state.player.z+2.15},{x:state.player.x+.50,y:state.player.y-.15,z:state.player.z+2.15});
      state.fireTimer=C.CONFIG.fireCooldown;A.playLaser();
    }
    if(state.spawnTimer<=0){state.enemies.push(C.makeEnemy(state.enemyId++));state.spawnBase=Math.max(.48,1.05-state.elapsed/110);state.spawnTimer=C.nextEnemySpawn(state.spawnBase);}
    ...
    for(const e of state.enemies){
      e.z-=e.speed*dt;e.x+=Math.sin(state.elapsed*1.9+e.phase)*dt*(e.kind==='dart'?1.5:.65);e.y+=Math.cos(state.elapsed*1.3+e.phase)*dt*.22;
      for(const l of state.lasers){
        if(e.alive&&C.spheresHit(e,l,1.15,2.8)){e.alive=false;l.z=999;state.score+=C.scoreForEnemy(e);burst(e.x,e.y,e.z);A.playExplosion();}
      }
    ...
    state.enemies=state.enemies.filter(e=>e.alive);state.lasers=state.lasers.filter(l=>l.z<100);
    for(const p of state.particles){p.x+=p.vx*dt;p.y+=p.vy*dt;p.z+=p.vz*dt;p.life-=dt;}
    state.particles=state.particles.filter(p=>p.life>0&&p.z>1);state.area=1+Math.floor(state.elapsed/35);
  }

  function burst(x,y,z,n=12){for(let i=0;i<n;i++)state.particles.push({x,y,z,vx:C.rand(-5,5),vy:C.rand(-5,5),vz:C.rand(-8,8),life:C.rand(.25,.65)});}
  function endGame(){state.running=false;state.gameOver=true;A.playGameOver();startPanel.innerHTML=`<h1>GAME OVER</h1><p>SCORE ${String(state.score).padStart(7,'0')}</p><p>R / Enter で再出撃</p><button id="restartButton">RETRY</button>`;startPanel.classList.remove('hidden');document.getElementById('restartButton').addEventListener('click',start);}

  function projectWorld(x,y,z){return C.projectChase3D({x,y,z},viewport,C.CONFIG.cameraFocal,CAMERA_PITCH);}
  ...
  function drawTexturedTriangle(texture,p0,p1,p2,uv0=UV[0],uv1=UV[1],uv2=UV[2],shade=0){
    ...
    ctx.strokeStyle='rgba(235,245,255,.12)';ctx.lineWidth=.7;ctx.beginPath();ctx.moveTo(p0.x,p0.y);ctx.lineTo(p1.x,p1.y);ctx.lineTo(p2.x,p2.y);ctx.closePath();ctx.stroke();
  }
  function drawMesh(model,obj,scale,texture){
    ...
    for(const item of faces){const[a,b,c]=item.face.v;drawTexturedTriangle(texture,pts[a],pts[b],pts[c],UV[0],UV[1],UV[2],item.face.s||0);}
  }
  ...
  function drawCapitalShip(){
    ...
    for(let z=z0+5;z<100;z+=9){drawWorldPolygon([[-.72,7.17,z],[-.32,7.17,z],[-.24,7.15,z+2.8],[-.58,7.15,z+2.8]],'#8bd7ff');drawWorldPolygon([[.32,7.17,z],[.72,7.17,z],[.58,7.15,z+2.8],[.24,7.15,z+2.8]],'#8bd7ff');}
    for(let z=z0;z<100;z+=12){drawWorldPolygon([[-9.6,7.25,z+1],[-5.3,7.25,z+1],[-5.0,7.20,z+8],[-9.0,7.20,z+8]],'#2c3541','#576474',.8);drawWorldPolygon([[5.3,7.25,z+1],[9.6,7.25,z+1],[9.0,7.20,z+8],[5.0,7.20,z+8]],'#29333e','#576474',.8);}
    const towers=[[-6.6,6.2,29-scroll],[6.2,6.05,40-scroll],[-7.0,5.95,61-scroll],[6.8,6.10,76-scroll]];
    for(const[x,y,z]of towers){const zz=((z-10+96)%96)+10;drawWorldPolygon([[x-1.4,7.1,zz-1.5],[x+1.4,7.1,zz-1.5],[x+1.1,7.0,zz+3],[x-1.1,7.0,zz+3]],'#46515d','#738091',1);drawWorldPolygon([[x-1,y,zz],[x+1,y,zz],[x+.7,y,zz+2.6],[x-.7,y,zz+2.6]],'#596675','#8894a2',1);drawLine3D([x,y-.25,zz+1],[x,y-1,zz+4.5],'#9aa8b7',2);}
  ...
  function drawPlayfield(){
    ...
    drawCapitalShip();drawRetroGrid();
    for(const l of state.lasers){const a=projectWorld(l.x,l.y,l.z),b=projectWorld(l.x,l.y,l.z+6);ctx.strokeStyle='#ff584f';ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(a.x,a.y);ctx.lineTo(b.x,b.y);ctx.stroke();}
    ...
    for(const p of state.particles){const q=projectWorld(p.x,p.y,p.z);ctx.fillStyle=p.life>.3?'#fff':'#ff8a4c';ctx.fillRect(q.x,q.y,3,3);}drawPlayer();ctx.restore();
  }
  function drawIdleStars(){ctx.fillStyle='#fff';for(let i=0;i<80;i++)ctx.fillRect((i*83)%PLAY_W,(i*47)%H,1,1);}
  function drawRetroGrid(){for(let z=12;z<=88;z+=8)drawLine3D([-11,5.8,z],[11,5.8,z],'rgba(83,126,168,.12)',1);for(let x=-10;x<=10;x+=2)drawLine3D([x,5.8,10],[x,5.8,90],'rgba(83,126,168,.08)',1);}
  function drawPlayer(){drawMesh(playerModel,state.player,1.18,textures.player);const la=projectWorld(state.player.x-.68,state.player.y+.12,state.player.z-1.65),lb=projectWorld(state.player.x-.68,state.player.y+.22,state.player.z-3.7),ra=projectWorld(state.player.x+.68,state.player.y+.12,state.player.z-1.65),rb=projectWorld(state.player.x+.68,state.player.y+.22,state.player.z-3.7);ctx.strokeStyle='#ff884d';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(la.x,la.y);ctx.lineTo(lb.x,lb.y);ctx.moveTo(ra.x,ra.y);ctx.lineTo(rb.x,rb.y);ctx.stroke();}

  function drawHUD(){
    ctx.fillStyle='#05070a';ctx.fillRect(HUD_X,0,W-HUD_X,H);ctx.strokeStyle='#c4c9d0';ctx.lineWidth=2;ctx.strokeRect(HUD_X+10,10,W-HUD_X-20,H-20);ctx.fillStyle='#e9edf3';ctx.textAlign='center';ctx.font='bold 18px monospace';ctx.fillText('SCORE',860,42);ctx.font='bold 26px monospace';ctx.fillStyle='#ff5555';ctx.fillText(String(state?.score||0).padStart(7,'0'),860,70);
