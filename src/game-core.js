(function (root, factory) {
  ...
  function makeEnemy(id, stageNumber = 1, rng = Math.random) {
    ...
    const kindRoll = rng(), patternRoll = rng();
    const kind = kindRoll > 0.78 ? 'dart' : kindRoll > 0.47 ? 'fighter' : 'interceptor';
    const patterns = ['weave', 'zigzag', 'sweep', 'hunter'];
    return {
      id, kind,
      pattern: patterns[Math.min(patterns.length - 1, Math.floor(patternRoll * patterns.length))],
      x: rand(-7.8, 7.8, rng), y: CONFIG.flightPlaneY, z: CONFIG.enemySpawnZ,
  ...
  function moveEnemyBullet(bullet, dt) {
    return { ...bullet, x:bullet.x+bullet.vx*dt, y:bullet.y+bullet.vy*dt, z:bullet.z+bullet.vz*dt };
  }
